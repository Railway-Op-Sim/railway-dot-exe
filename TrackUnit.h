...
/// True if another train on LinkPos track of element at TrackPos, whether bridge or not, return false if not, or if TrackPos == -1, or LinkPos == -1, or if only own train on the track
...
/// checks if the track that the train is on is long enough for a split, returns false if not, or if points set wrongly or a train obstructing the split
    bool ThisLocationLongEnoughForSplit(int Caller, AnsiString HeadCode, int TrainID, AnsiString LocationName, int LeadElement, int LeadExitPos,
        int MidElement, int MidEntryPos, int &FrontTrainFrontPos, int &FrontTrainRearPos, int &RearTrainFrontPos, int &RearTrainRearPos, bool &TemporaryDelay);
...
/// Used when setting signal aspects for a route by working forwards through the route to see what the next forward signal aspects is, because this determines all the rearward signal aspects (added StartPos at v2.17.0 as function now called in SetRearwardsSignalsReturnFalseForTrainInRear instead of SetRouteSignals)
...
/// Called by TAllRoutes::SetAllRearwardsSignals to set rearwards signals from a specified starting position.  If a train is found during the rearwards search then this function flags the fact so that the calling function can change its behaviour with respect to further rearwards signal aspects (added SkipForwardLook at v2.17.0 so behaviour different in first and second calls).
